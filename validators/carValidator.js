const { body } = require('express-validator');
    ...
    const carJsonValidate = [
        body('vin_number').trim().isLength({min:17,max:17}).withMessage('length of vin_number is incorrect'), //ok
        body('car_performance').trim().isInt({min:20, max:1200}).withMessage('car performance must be between 20 and 1200'), //ok
        body('engine_size').trim().isInt({min:250,max:10000}).withMessage('engine size must be between 250 and 10000'),
        body('production_time').trim().matches('/^(?:[1-9]{4}-[0-9]{2}-[0-9]{2})$/').withMessage('this field is not date format'),
        ...
        body('type').trim().isLength({min:5,max:20}).withMessage('length of vin_number is incorrect')
    ];
